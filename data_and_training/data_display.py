...
Detection mode  (stdin carries data — reads JSON from stdin):
    python infer.py IMAGE_PATH | python data_and_training/data_display.py DS_DIR IMAGE_STEM
...
The inference script writes one JSON object per image:

    {
      "image": "0035a4bfeda1b637.jpg",
      "detections": [
...
The "image" field identifies the source image.

The "detections" field contains:
    ...
    score  float  Confidence score [0, 1].  Key is optional.

For backward compatibility, the old raw JSON array format is also accepted:

    [
      {"name": "apple", "score": 0.91, "x": 100, "y": 50, "w": 200, "h": 150}
    ]

Note that data_display.py is intended to display one image at a time.
When using the directory mode of infer.py, a downstream streaming tool
should first select/extract the record for the desired image.
...
third-party : matplotlib >= 3.7 (tested with 3.10.9), Pillow
"""
...
def _find_image(ds_dir: Path, stem: str) -> Path:
    ...
    raise FileNotFoundError(
        f"No .jpg/.jpeg found for '{stem}' in {ds_dir / 'images'}"
    )
...
def _load_detections() -> list[dict]:
    """
    Load one image's inference result from stdin.
    ...
    The old raw array format is also accepted for compatibility.
    """
    ...
    if isinstance(data, dict):
        ...
        # Backward compatibility with the old infer.py format.
        detections = data
    else:
        raise ValueError(
            "Expected inference JSON object or array on stdin."
        )
...
def _show(image_path: Path, boxes: list[dict], title: str) -> None:
    ...
    # Render to a temp file and delegate display to PIL.
